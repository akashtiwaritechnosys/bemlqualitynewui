...
class HelpDesk_DescribeModuleForSR_Action extends Vtiger_Action_Controller {
	...
	public function process(Vtiger_Request $request) {
		...
		if ($current_user) {
			...
			$dependencyFields = array('purpose', 'manual_equ_ser','pincode', 'city','pre_address','state');
			foreach ($describeInfo['fields'] as $key => $value) {
				if (in_array($value['name'], $dependencyFields)) {
					...
					} elseif (in_array($value['name'],array('pincode', 'city','pre_address','state'))){
						$value['dependentOnMasterValue'] = array('PREVENTIVE MAINTENANCE', 'PERIODICAL MAINTENANCE','BREAKDOWN','SERVICE FOR SPARES PURCHASED','GENERAL INSPECTION');
						$value['dependentField'] = true;
						$value['initialDisplay'] = false;
