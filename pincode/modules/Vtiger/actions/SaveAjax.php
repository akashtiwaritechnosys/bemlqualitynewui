...
class Vtiger_SaveAjax_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		try {
			...
			if($request->get('module') == 'ProjectTask' && $request->get('field') == 'projecttaskstatus' && $request->get('value') == 'Completed'){
				$recordModel = Vtiger_Record_Model::getInstanceById($request->get('record'));
				$previousTask = $recordModel->get('cf_862');
				if((int)$previousTask != 0){
					$previousTaskInstance = Vtiger_Record_Model::getInstanceById($previousTask);
					if($previousTaskInstance->get("projecttaskstatus") != 'Completed'){
						$response->setError("Prevois Task is not Completed");
					}  else {
