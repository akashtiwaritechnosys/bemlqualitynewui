...
class Mobile_WS_PreUserSignUp extends Mobile_WS_Controller {
    ...
    function process(Mobile_API_Request $request) {
        ...
        $badgeNoExits = $this->isBadgeExits($request->get('badge_no'));
        if($badgeNoExits){
        ...
        if (!empty($emailId)) {
            ...
            $handler_data['time'] = strtotime("+5 Minute");
            ...
            $content = 'Dear Customer,<br><br> 
                ...
                <br><br> 
                This request was made on ' . date("Y-m-d H:i:s") . ' and will expire in next 5 Minute.<br><br> 
                Regards,<br> 
                CRM Support Team.<br>';
    ...
    function isBadgeExits($badgeNo) {
        global $adb;
        $sql = 'select 1 from vtiger_serviceengineer where badge_no = ?';
        $sqlResult = $adb->pquery($sql, array(trim($badgeNo)));
