...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='')
{
	...
	for($i=1; $i<=$tot_no_prod; $i++)
	{
		...
		if ($module == 'ServiceReports') {
			$srAction1 = vtlib_purify($_REQUEST['sr_action_one' . $i]);
			$srAction2 = vtlib_purify($_REQUEST['sr_action_two' . $i]);
			$sr_replace_action = vtlib_purify($_REQUEST['sr_replace_action' . $i]);
		}
		...
		if ($module == 'ServiceReports') {
			$query = 'INSERT INTO vtiger_inventoryproductrel(id, productid, productname, sequence_no, quantity, listprice, comment, description, purchase_cost, margin, dependency,sr_action_one,sr_action_two,sr_replace_action)
					VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
			$qparams = array($focus->id, $prod_id, $prod_name, $prod_seq, $qty, $listprice, $comment, $description, $purchaseCost, $margin, $dependent,$srAction1,$srAction2,$sr_replace_action);
		} else {
