...
function getAssociatedProducts($module, $focus, $seid = '', $refModuleName = false)
{
	...
	for($i=1;$i<=$num_rows;$i++)
	{
		...
		$sr_action_one = $adb->query_result($result, $i - 1, 'sr_action_one');
		$sr_action_two = $adb->query_result($result, $i - 1, 'sr_action_two');
		$sr_replace_action = $adb->query_result($result, $i - 1, 'sr_replace_action');
		...
		$product_Detail[$i]['sr_action_one'.$i] = from_html($sr_action_one);
		$product_Detail[$i]['sr_action_two'.$i] = from_html($sr_action_two);
		$product_Detail[$i]['sr_replace_action'.$i] = from_html($sr_replace_action);
