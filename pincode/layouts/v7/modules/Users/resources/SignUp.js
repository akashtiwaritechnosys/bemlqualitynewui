	...
	registerEvents: function () {
		...
		jQuery(document).ready(function () {
			...
			togglePassword1.addEventListener('click', function (e) {
				...
				//$('#togglePassword1').toggleClass("glyphicon-eye-close");
			...
			var params = {
				submitHandler: function (form) {
					...
					if(numberStatus == false){
						app.helper.showErrorNotification({ message: 'Mobile NUmber Is Not Valid' });
						return;
					}else{
						$('#phone').val(num);
					}
					...
					if (user_password != confirm_password) {
						app.helper.showErrorNotification({ message: 'Set Password And Re-Type Password Is Not Matching' });
						var result = false;
						return result;
					}
					...
					$.ajax({
						...
						success: function (response) {
							if (response.success == true) {
								...
								app.helper.showErrorNotification({ message: app.vtranslate(response.error.message) });
							}
