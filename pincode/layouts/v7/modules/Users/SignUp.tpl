		...
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 login_beml">
			<form class="form-horizontal" id="SignUpFormDiv" name="SignUpFormDiv">
				<div class="loginDiv widgetHeight ">
							...
							<div>
								...
								{foreach from=$USERBLOCKS item=blockitem key=blockkey}
									...
									<div class="">
										...
										{elseif $item['name'] eq 'cust_role' || $item['name'] eq 'designaion' }
											<span id="{{$item['name']}}" {if $item['initialDisplay'] eq 'no'} style="display: none" {/if}>
												...
												<div class="col-sm-8 form-group">
													<select {if $item['mandatory'] eq true} data-rule-required="true"{/if} id="{{$item['name']}}1" onchange="myFunction(event)" id="{{$item['name']}}"  data-field="{$item|@json_encode}" class="form-control" data-fieldtype="picklist"  name="{{$item['name']}}"  portal-select>
														<option value="">
														</option>
													</select>
												</div>
										...
										{elseif $item['type']['name'] eq 'picklist' }
											<span id="{{$item['name']}}" {if $item['initialDisplay'] eq 'no'} style="display: none" {/if}>
												<label for="{{$item['name']}}" class="col-sm-4 control-label">{{$item['label']}} {if $item['mandatory'] eq true} <span class="text-danger">*</span>{/if}</label>
												<div class="col-sm-8 form-group">
													<select {if $item['mandatory'] eq true} data-rule-required="true"{/if} onchange="myFunction(event)" id="{{$item['name']}}"  data-field="{$item|@json_encode}" class="form-control" data-fieldtype="picklist"  name="{{$item['name']}}"  portal-select>
														<option value="">
														</option>
										...
										{elseif $item['type']['name'] eq 'phone'}
											...
											<h5 style="text-align: center;margin-top: 1px;margin-bottom: 5px;">(OTP Will Be Sent To Verify your Mobile Number)</h5>
											<label for="{{$item['name']}}" class="col-sm-4 control-label">{{$item['label']}} {if $item['mandatory'] eq true} <span class="text-danger">*</span>{/if}</label>
											<div class="col-sm-8 form-group">
												<input {if $item['mandatory'] eq true} data-rule-required="true"{/if} {literal}data-specific-rules='[{"name":"itivalidate"}]'{/literal}  id="phone" type="tel" placeholder="" class="form-control"  name="{{$item['name']}}"  autofill="autofill">
											</div>
										...
										{elseif $item['type']['name'] eq 'integer'}
											<span id="{{$item['name']}}" {if $item['initialDisplay'] eq 'no'}style="display: none" {/if}>
												<label for="{{$item['name']}}" class="col-sm-4 control-label">{{$item['label']}}  {if $item['mandatory'] eq true} <span class="text-danger">*</span>{/if}</label>
												<div class="col-sm-8 form-group">
													<input {literal}data-specific-rules='[{"name":"integer"},{"name" : "maxsize", "params":"5"},{"name" : "minsize", "params":"5"}]'{/literal} {if $item['mandatory'] eq true} data-rule-required="true"{/if} type="text" class="form-control"  name="{{$item['name']}}" autofill="autofill">
												</div>
											</span>
										{elseif $item['name'] eq 'user_password'}
											<span id="{{$item['name']}}" {if $item['initialDisplay'] eq 'no'}style="display: none" {/if}>
												<label for="{{$item['name']}}" class="col-sm-4 control-label">{{$item['label']}}  {if $item['mandatory'] eq true} <span class="text-danger">*</span>{/if}</label>
												<div class="col-sm-8 form-group">
													<input id="password_1" {if $item['mandatory'] eq true} data-rule-required="true"{/if} {literal}data-specific-rules='[{"name":"passwordmix"},{"name" : "maxsize", "params":"18"}]'{/literal} type="password" class="form-control"  name="{{$item['name']}}" autofill="autofill">
										...
										{elseif $item['name'] eq 'confirm_password'}
											<span id="{{$item['name']}}" {if $item['initialDisplay'] eq 'no'}style="display: none" {/if}>
												<label for="{{$item['name']}}" class="col-sm-4 control-label">{{$item['label']}}  {if $item['mandatory'] eq true} <span class="text-danger">*</span>{/if}</label>
												<div class="col-sm-8 form-group">
													<input id="password_2" {if $item['mandatory'] eq true} data-rule-required="true"{/if} {literal}data-specific-rules='[{"name":"passwordmix"},{"name" : "maxsize", "params":"18"}]'{/literal} type="password" class="form-control"  name="{{$item['name']}}" autofill="autofill">
					...
					<div class="submit_btn form-group" style="padding : 20px">
						<div class="col-sm-12">
							<button id="SignUPSubmitButton" class="button buttonBlue crm_btn btn btn-primary">Sign Up</button><br>
							<br>
						     <br>
							<a href = "index.php?module=Users&parent=Settings&view=Login&error=login" class="btn  btn-primary" ><span class="glyphicon glyphicon-chevron-left"></span>  Back </a>
						</div>
					</div>
				</div>
			</form>
