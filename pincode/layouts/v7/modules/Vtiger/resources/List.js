...
Vtiger.Class("Vtiger_List_Js", {
	...
	syncToSAPMaterialMaster : function () {
		app.helper.showProgress();
		this.getSAPAPICallMaterialMaster().then(
				function (data) {
					app.helper.hideProgress();
				},
				function (error, err) {
	...
	syncToSAPEquipment : function () {
		app.helper.showProgress();
		this.getSAPAPICallEquipment().then(
				function (data) {
					app.helper.hideProgress();
				},
				function (error, err) {
	...
	getSAPAPICallEquipment : function () {
        var aDeferred = jQuery.Deferred();
        var url = "module=Equipment&action=getAllEquipment";
        AppConnector.request(url).then(
                function (data) {
                    if (data['success']) {
                        aDeferred.resolve(data);
	...
	getSAPAPICallMaterialMaster : function () {
        var aDeferred = jQuery.Deferred();
        var url = "module=Products&action=ProductsExternalAppSync";
        AppConnector.request(url).then(
                function (data) {
                    if (data['success']) {
                        aDeferred.resolve(data);
	...
	initListView : function () {
		...
		thisInstance.getPageCount().then(function (data) {
			...
			let  currentPage  = parseInt(currentPageElement.val());
			...
			for(let i = 0 ; i < rangeLength; i++) {
				if(l != 0) {
					if (range[i] - l === 2) {
						...
						if((l + 1) == currentPage){
							kamati = kamati + '<button  class="listpageactive btn btn-soft-secondary" name='+ (l + 1) +' id="aaa">'+ (l + 1) +'</button>';
						} else {
							kamati = kamati + '<button  class="btn btn-soft-secondary" name='+ (l + 1) +' id="aaa">'+ (l + 1) +'</button>';
						}
						
					} else if (range[i] - l !== 1) {
						// rangeWithDots.push('...');
						kamati = kamati + '<button class="btn btn-soft-secondary">'+ '...' +'</button>';
					}
				}
				if((range[i]) == currentPage){
					kamati = kamati + '<button class="listpageactive btn btn-soft-secondary" name='+ (range[i]) +' id="aaa">'+ range[i] +'</button>';
				} else {
					kamati = kamati + '<button class="btn btn-soft-secondary" name='+ (range[i]) +' id="aaa">'+ range[i] +'</button>';
				}
				// rangeWithDots.push(range[i]);
				l = range[i];
			}
			igDiv = igDiv + kamati + "</div>";
			var giliment = listViewContainer.find('#PageJump');
			giliment.html(igDiv);

			let container1 = jQuery('#kamati');
			container1.on('click', '#aaa', function (e) {
				var currentElement = jQuery(e.currentTarget);
				var urlParams = thisInstance.getPagingParams();
				urlParams['page'] = currentElement.attr('name');
