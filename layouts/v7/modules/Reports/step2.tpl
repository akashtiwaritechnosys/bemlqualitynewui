...
{strip}
    <form class="form-horizontal recordEditView" id="report_step2" method="post" action="index.php">
        ...
        {if $type eq 'Pivot'}
            <div class="" style="border:1px solid #ccc;padding:4%;height: 200px !important;">
                <div style="position:relative">
                    <div class="form-group" style="float:left; width: 30%;margin-right: 45px;">
                        <label>{vtranslate('LBL_SELECT_ROWS',$MODULE)}({vtranslate('LBL_MAX',$MODULE)} 3)</label><br>
                        <select data-placeholder="{vtranslate('LBL_ADD_MORE_COLUMNS',$MODULE)}" name="pivot_rows" id="reportsColumnsList" style="width :100%;" class="select2-container select2 col-lg-11 columns"  data-rule-required="true" multiple="">
                                ...
                                {foreach key=BLOCK_LABEL item=BLOCK from=$PRIMARY_MODULE}
                                    <optgroup label='{vtranslate($PRIMARY_MODULE_NAME,$MODULE)}-{vtranslate($BLOCK_LABEL,$PRIMARY_MODULE_NAME)}'>
                                        {foreach key=FIELD_KEY item=FIELD_LABEL from=$BLOCK}
                                            <option value="{$FIELD_KEY}" {if $pivotColumns[$FIELD_KEY] eq $FIELD_KEY} selected {/if}>{vtranslate($PRIMARY_MODULE_NAME, $PRIMARY_MODULE_NAME)} {vtranslate($FIELD_LABEL, $PRIMARY_MODULE_NAME)}</option>
                                        {/foreach}
                                    </optgroup>
                                {/foreach}
                            {/foreach}
                            {foreach key=SECONDARY_MODULE_NAME item=SECONDARY_MODULE from=$SECONDARY_MODULE_FIELDS}
                                {foreach key=BLOCK_LABEL item=BLOCK from=$SECONDARY_MODULE}
                                    <optgroup label='{vtranslate($SECONDARY_MODULE_NAME,$MODULE)}-{vtranslate($BLOCK_LABEL,$SECONDARY_MODULE_NAME)}'>
                                        {foreach key=FIELD_KEY item=FIELD_LABEL from=$BLOCK}
                                            <option value="{$FIELD_KEY}" {if $pivotColumns[$FIELD_KEY] eq $FIELD_KEY} selected {/if}>{vtranslate($SECONDARY_MODULE_NAME, $SECONDARY_MODULE_NAME)} {vtranslate($FIELD_LABEL, $SECONDARY_MODULE_NAME)}</option>
                                        {/foreach}
                                    </optgroup>
                                {/foreach}
                            {/foreach}
                    ...
                    <div class="form-group" style="float:left; width: 30%;margin-right: 45px;">
                        <label>{vtranslate('LBL_SELECT_COLUMNS',$MODULE)}({vtranslate('LBL_MAX',$MODULE)} 3)</label><br>
                        <select data-placeholder="{vtranslate('LBL_ADD_MORE_COLUMNS',$MODULE)}" name="pivot_columns" id="reportsRowList" style="width :100%;" class="select2-container select2 col-lg-11 columns"  data-rule-required="true" multiple="">
                            {foreach key=PRIMARY_MODULE_NAME item=PRIMARY_MODULE from=$PRIMARY_MODULE_FIELDS}
                                {foreach key=BLOCK_LABEL item=BLOCK from=$PRIMARY_MODULE}
                                    <optgroup label='{vtranslate($PRIMARY_MODULE_NAME,$MODULE)}-{vtranslate($BLOCK_LABEL,$PRIMARY_MODULE_NAME)}'>
                                        {foreach key=FIELD_KEY item=FIELD_LABEL from=$BLOCK}
                                            <option value="{$FIELD_KEY}" {if $pivotRows[$FIELD_KEY] eq $FIELD_KEY} selected {/if}>{vtranslate($PRIMARY_MODULE_NAME, $PRIMARY_MODULE_NAME)} {vtranslate($FIELD_LABEL, $PRIMARY_MODULE_NAME)}</option>
                                        {/foreach}
                                    </optgroup>
                                {/foreach}
                            {/foreach}
                            {foreach key=SECONDARY_MODULE_NAME item=SECONDARY_MODULE from=$SECONDARY_MODULE_FIELDS}
                                {foreach key=BLOCK_LABEL item=BLOCK from=$SECONDARY_MODULE}
                                    <optgroup label='{vtranslate($SECONDARY_MODULE_NAME,$MODULE)}-{vtranslate($BLOCK_LABEL,$SECONDARY_MODULE_NAME)}'>
                                        {foreach key=FIELD_KEY item=FIELD_LABEL from=$BLOCK}
                                            <option value="{$FIELD_KEY}" {if $pivotRows[$FIELD_KEY] eq $FIELD_KEY} selected {/if}>{vtranslate($SECONDARY_MODULE_NAME, $SECONDARY_MODULE_NAME)} {vtranslate($FIELD_LABEL, $SECONDARY_MODULE_NAME)}</option>
                                        {/foreach}
                                    </optgroup>
                                {/foreach}
                            {/foreach}
                    ...
                    <div class="form-group" style="float:left; width: 30%;margin-right: 45px;">
                        <label>{vtranslate('Select Data Fields',$MODULE)}({vtranslate('LBL_MAX',$MODULE)} 3)</label><br>
                        {assign var=FIELD_OPERATION_VALUES value=','|explode:'SUM:2,AVG:3,MIN:4,MAX:5'}
                        <select data-placeholder="{vtranslate('Click to select Data Fields',$MODULE)}" class="select2" name="pivot_datafield" id="operationField" style="width :100%;" class="select2-container select2 col-lg-11 columns"  data-rule-required="true" multiple="">
                            {foreach item=operationFieldValues_VALUE key=operationFieldValues_KEY from=$operationFieldValues}
                                <option value="{$operationFieldValues_KEY}" {if $pivotDatafield[$operationFieldValues_KEY] eq $operationFieldValues_KEY} selected {/if}>{$operationFieldValues_VALUE}</option>
                            {/foreach}
                        </select>
                    </div>
                </div>
