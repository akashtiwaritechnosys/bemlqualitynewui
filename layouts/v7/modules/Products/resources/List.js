Vtiger_List_Js("Products_List_Js", {
    syncAltMaterials : function (index) {
        let self = this;
		app.helper.showProgress();
        if(index == 1){
            index = parseInt(localStorage.getItem('LastScaneedALTProductIndex'));
            if(index == undefined || index == null || isNaN(index)){
                index = 1;
            }
        }
		this.getSAPALTAPICall(index).then(
				function (data) {
                    if(data.result.hasNext == true){
                        localStorage.setItem('LastScaneedALTProductIndex',index);
                        self.getSAPALTAPICall(index + 1);
                    } else {
    ...
    getSAPALTAPICall: function (index) {
        var aDeferred = jQuery.Deferred();
        var url = "module=Products&action=SyncAlterNativeParts&indexForLink="+index;
