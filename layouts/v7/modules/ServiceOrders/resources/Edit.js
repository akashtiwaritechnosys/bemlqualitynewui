Inventory_Edit_Js("ServiceOrders_Edit_Js", {}, {
    registerBasicEvents: function (container) {
        ...
        self.lineItemsHolder.find('tr.' + self.lineItemDetectingClass).each(function (index, domElement) {
            ...
            if (val == false) {
                ...
                lineItemRow.find('.manual_part_no_so').removeClass('hide');
            }
        });
    },
                            ...
                            app.request.get({ 'url': dataUrl }).then(
                                function (error, data) {
                                    if (error == null) {
                                        ...
                                        data['0'][objKeys['0']]['manual_part_no_so'] = $("#manual_part_no" + rowNum).val();
    ...
    mapResultsToFields: function (parentRow, responseData) {
        ...
        parentRow.find('.manual_part_no_so').removeClass('hide');
        parentRow.data('.manual_part_no_so', recordData.manual_part_no_so);
        jQuery('textarea.manual_part_no_so', parentRow).val(recordData.manual_part_no_so);
    },
