{strip}
    ...
    <div id="paymentContainer" style="margin : 0px" name="paymentContainer" class="paymentOptions">
        {foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$PICKLIST_VALUES}
            ...
            <label> <input id="{$FIELD_MODEL->getFieldName()}{Vtiger_Util_Helper::replaceSpaceWithUnderScores(Vtiger_Util_Helper::toSafeHTML($PICKLIST_NAME))}" {if $FIELD_INFO["mandatory"] eq true} data-rule-required="true" {/if} data-fieldname="{$FIELD_MODEL->getFieldName()}" name="{$FIELD_MODEL->getFieldName()}[]" type="checkbox" value="{Vtiger_Util_Helper::toSafeHTML($PICKLIST_NAME)}" {if in_array(Vtiger_Util_Helper::toSafeHTML($PICKLIST_NAME), $FIELD_VALUE_LIST)} checked="checked" {/if}>
