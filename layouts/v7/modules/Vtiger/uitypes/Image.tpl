	...
	{else}
		...
		<div class="fileUploadContainer text-left">
			<div class="fileUploadBtn btn btn-primary">
				...
				<input type="file"
					class="inputElement {if $MODULE eq 'Products' or $MODULE eq 'HelpDesk' or $MODULE eq 'ServiceReports' or $MODULE eq 'InternalTickets'}multi max-6{/if} {if $FIELD_MODEL->get('fieldvalue') and $FIELD_INFO["mandatory"] eq true} ignore-validation {/if}"
					name="{$FIELD_MODEL->getFieldName()}[]" value="{$FIELD_MODEL->get('fieldvalue')}"
					{if !empty($SPECIAL_VALIDATOR)}data-validator="{Zend_Json::encode($SPECIAL_VALIDATOR)}" {/if}
					{if $FIELD_INFO["mandatory"] eq true} data-rule-required="true" {/if} {if count($FIELD_INFO['validator'])}
			...
			{if $FIELD_MODEL->getName() eq $IMAGE_INFO.fieldNameFromDB}
				{if !empty($IMAGE_INFO.url)}
					{if ( strpos($IMAGE_INFO.name , '.png') >= 1 || strpos($IMAGE_INFO.name , '.jpeg') >= 1 || strpos($IMAGE_INFO.name , '.jpg') >= 1 || strpos($IMAGE_INFO.name , '.JPEG') >= 1 )  }
						<span class="col-lg-6" name="existingImages"><img src="{$IMAGE_INFO.url}" data-image-id="{$IMAGE_INFO.id}"
								width="400" height="250"></span>
						<span class="col-lg-3">
							<span class="row">
								<span class="col-lg-11">[{$IMAGE_INFO.name}]</span>
								<span class="col-lg-1"><input type="button" id="file_{$ITER}" value="{vtranslate('LBL_DELETE','Vtiger')}"
										class="imageDelete"></span>
							</span>
						</span>
					{else}
						<span class="col-lg-6" name="existingImages"> <img style="display:none" data-image-id="{$IMAGE_INFO.id}"
								target="_blank" title="{$IMAGE_INFO.name}" /><span><a
									href="{$IMAGE_INFO.url}">{$IMAGE_INFO.name}</a></span></span>
						<span class="col-lg-3">
							<span class="row">
								<span class="col-lg-11">[{$IMAGE_INFO.name}]</span>
								<span class="col-lg-1"><input type="button" id="file_{$ITER}" value="{vtranslate('LBL_DELETE','Vtiger')}"
