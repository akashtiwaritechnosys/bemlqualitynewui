...
{strip}
    ...
    <div id="cc_row_{$cc_i}" class="cc_row_class col-lg-12" style="padding:5px 0px 5px 0px;">
        ...
        <input type="text" name="cc_name_{$cc_i}" id="cc_name_{$cc_i}" class="inputElement col-lg-2 cc_row_name" value="{if !empty($cc_calculation_arr)}{$cc_calculation_arr.cc_name}{/if}" placeholder="{vtranslate('LBL_CUSTOM_CALCULATION_NAME', $MODULE)}" >
        </div>
        <div class="col-lg-2">
        ...
        {if !empty($cc_calculation_arr)}{$cc_calculation_arr.cc_calculation}{/if}
        </textarea>
        </div>
        <div class="col-lg-2">

            <input type="hidden" id="cc_totals_hidden_{$cc_i}" name="cc_totals_hidden_{$cc_i}" value="{if !empty($cc_calculation_arr)}{$cc_calculation_arr.cc_totals_hidden}{/if}" class="cc-totals-hidden">
            <select name="cc_totals_{$cc_i}" id="cc_totals_{$cc_i}" multiple class=" {$cc_special} inputElement cc-new-select cc-totals"  >
                <option value="sum" {if !empty($cc_calculation_arr) && in_array('sum', $cc_calculation_arr.cc_totals)}selected{/if} >{vtranslate('SUM',$MODULE)}</option>
                <option value="avg" {if !empty($cc_calculation_arr) && in_array('avg', $cc_calculation_arr.cc_totals)}selected{/if} >{vtranslate('AVG',$MODULE)}</option>
                <option value="min" {if !empty($cc_calculation_arr) && in_array('min', $cc_calculation_arr.cc_totals)}selected{/if} >{vtranslate('MIN',$MODULE)}</option>
                <option value="max" {if !empty($cc_calculation_arr) && in_array('max', $cc_calculation_arr.cc_totals)}selected{/if} >{vtranslate('MAX',$MODULE)}</option>
            </select>
        </div>
        <div class="col-lg-2">
        <span>
