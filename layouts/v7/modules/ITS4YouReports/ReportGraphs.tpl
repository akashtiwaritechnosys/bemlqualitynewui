...
{strip}
    ...
    <div style="border:1px solid #ccc;padding:4%;">
        ...
        <div class="form-group">
            <label class="control-label textAlignLeft col-lg-2">
                {vtranslate('Chart Progress Line',$MODULE)}
            </label>
            <div class="col-lg-4">
                <input type="hidden" name="progress_line_val" id="progress_line_val" value="{$progress_line}">
                <select class="select2 inputElement" id="progress_line" name="progress_line" multiple="multiple" >
                    {foreach item=lineName from=['MIN', 'AVG', 'MAX']}
                        <option value="{$lineName}" {if in_array($lineName, $progress_line_array)}selected{/if} >{$lineName|@getTranslatedString:'ITS4YouReports'}</option>
                    {/foreach}
                </select>
            </div>
        </div>
