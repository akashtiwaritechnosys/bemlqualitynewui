...
$( document ).ready(function() {
    ...
    <div style="padding: 40px">
        ...
        <table id="data_table">
            <tr>
                <th class="NoBackGround" rowspan="1">Non CIL SUBSIDIERIES</th>
                        ...
                        {foreach item=INNER_VALUE1 key=INNER_KEY1 from=$NAME_FIELD}
                            ...
                            {else}
                                {if $KEY_FIELD eq 'Grand Total'}
                            		<th rowspan="1" class="NoBackGround" class="NoBackGround">{$INNER_VALUE1}</th>
                            	{else}
                            		<th rowspan="1" class="NoBackGround" class="NoBackGround"><a href="index.php?module=Equipment&view=List&orderby=&sortorder=&app=INVENTORY&search_params={$ListViewURL[$KEY_FIELD][$INNER_KEY1]}" target="_blank">{$INNER_VALUE1}</a></th>
                            	{/if}
                            {/if}
                        {/foreach}
                    </tr>
