Inventory_Edit_Js("ReturnSaleOrders_Edit_Js", {
	handleQtyValidations: function (event) {
		...
		let office = jQuery('#office').val();
		let cust_role = jQuery('#cust_role').val();
		if (office == 'Service Centre' && cust_role == 'Service Manager') {
			lineItemsHolder.find('tr.' + 'lineItemRow').each(function (index, domElement) {
				let lineItemRow = jQuery(domElement);
				let recivedty = parseFloat(lineItemRow.find('input[data-extraname="receivedvd_qty"]').val());
				let scm_dismant_unprogre = parseFloat(lineItemRow.find('input[data-extraname="scm_dismant_unprogre"]').val());
				let scm_item_aw_for_rep = parseFloat(lineItemRow.find('input[data-extraname="scm_item_aw_for_rep"]').val());
				let scm_repaired_qty = parseFloat(lineItemRow.find('input[data-extraname="scm_repaired_qty"]').val());
				let scm_rep_and_sent_back_qty = parseFloat(lineItemRow.find('input[data-extraname="scm_rep_and_sent_back_qty"]').val());
				let scm_beyond_eco_rep_qty = parseFloat(lineItemRow.find('input[data-extraname="scm_beyond_eco_rep_qty"]').val());
				let scm_senttoreg_worep = parseFloat(lineItemRow.find('input[data-extraname="scm_senttoreg_worep"]').val());

				let totalActionTakenQty =
					scm_dismant_unprogre + scm_item_aw_for_rep + scm_repaired_qty +
					scm_rep_and_sent_back_qty + scm_beyond_eco_rep_qty + scm_senttoreg_worep;

				if (recivedty != totalActionTakenQty) {
					app.helper.showErrorNotification({
						'message':
							`Received Qty And Total Of Present Status Qty Should Be Same,
							Received Qty - ` + recivedty + `
							Total Of Present Status Qty - ` + totalActionTakenQty + `
							`
					},{
						'delay' : 1000000000000
					});
		...
		} else if (office == 'Production Division' && cust_role == 'Divisonal Service Support') {
			lineItemsHolder.find('tr.' + 'lineItemRow').each(function (index, domElement) {
				let lineItemRow = jQuery(domElement);
				let recivedty = parseFloat(lineItemRow.find('input[data-extraname="receivedvd_qty"]').val());
				let ana_done_div_qty = parseFloat(lineItemRow.find('input[data-extraname="ana_done_div_qty"]').val());
				let und_fail_ana_div_qty = parseFloat(lineItemRow.find('input[data-extraname="und_fail_ana_div_qty"]').val());
				let asent_to_ven_qty = parseFloat(lineItemRow.find('input[data-extraname="asent_to_ven_qty"]').val());

				let totalActionTakenQty =
				ana_done_div_qty + und_fail_ana_div_qty + asent_to_ven_qty;

				if (recivedty < totalActionTakenQty) {
					app.helper.showErrorNotification({
						'message':
							`Total Of Present Status Qty Should Be Less Than Received Qty. 
							Received Qty - ` + recivedty + `
							Total Of Present Status Qty - ` + totalActionTakenQty + `
							`
					},{
				...
				let at_under_rep_atdiv = parseFloat(lineItemRow.find('input[data-extraname="at_under_rep_atdiv"]').val());
				let at_rep_a_kept_float = parseFloat(lineItemRow.find('input[data-extraname="at_rep_a_kept_float"]').val());
				let at_rep_a_sent_reg = parseFloat(lineItemRow.find('input[data-extraname="at_rep_a_sent_reg"]').val());
				let at_scraped_at_dev = parseFloat(lineItemRow.find('input[data-extraname="at_scraped_at_dev"]').val());

				let totalActionTakenByDSM =
				at_under_rep_atdiv + at_rep_a_kept_float + at_rep_a_sent_reg + at_scraped_at_dev;

				if (ana_done_div_qty > 0 && ana_done_div_qty != totalActionTakenByDSM) {
					app.helper.showErrorNotification({
						'message':
							`Analysis Done at Division Qty Should Be Same As Total Action Taken by DSM Qty. 
							Analysis Done at Division Qty - ` + ana_done_div_qty + `
							Total Action Taken by DSM Qty - ` + totalActionTakenByDSM + `
							`
					},{
						'delay' : 1000000000000
					});
		...
		lineItemsHolder.find('tr.' + 'lineItemRow').each(function (index, domElement) {
			...
			let val = parentRow.find('input[data-extraname="ana_done_div_qty"]').val();
			if (val > 0) {
				parentRow.find('#analysis_done_docDivCla').removeClass('hide');
				parentRow.find('#at_under_rep_atdivDivCla').removeClass('hide');
				parentRow.find('#at_rep_a_kept_floatDivCla').removeClass('hide');
				parentRow.find('#at_rep_a_sent_regDivCla').removeClass('hide');
				parentRow.find('#at_scraped_at_devDivCla').removeClass('hide');
			} else {
				parentRow.find('#analysis_done_docDivCla').addClass('hide');
				parentRow.find('#at_under_rep_atdivDivCla').addClass('hide');
				parentRow.find('#at_rep_a_kept_floatDivCla').addClass('hide');
				parentRow.find('#at_rep_a_sent_regDivCla').addClass('hide');
				parentRow.find('#at_scraped_at_devDivCla').addClass('hide');

				parentRow.find('.at_under_rep_atdiv').val(0);
				parentRow.find('.at_under_rep_atdiv').val(0);
				parentRow.find('.at_rep_a_kept_float').val(0);
				parentRow.find('.at_rep_a_sent_reg').val(0);
				parentRow.find('.at_scraped_at_dev').val(0);
			}
		});
	}
}, {
	...
	handleDefaultDependencyINRSO: function () {
		...
		self.lineItemsHolder.find('tr.' + self.lineItemDetectingClass).each(function (index, domElement) {
			...
			if (val == "Scrapped at Region") {
				parentRow.find('select.rso_part_status option').addClass('hide');
				parentRow.find('select.rso_part_status option[value="Scrapped at Region-Closed"]').removeClass('hide');
			} else if (val == "Sent to division to Analysis") {
				parentRow.find('select.rso_part_status option').addClass('hide');
				parentRow.find('select.rso_part_status option[value="STO Awaited From Division"]').removeClass('hide');
				parentRow.find('select.rso_part_status option[value="Waiting for Shipment From Region"]').removeClass('hide');
				parentRow.find('select.rso_part_status option[value="Dispatched from region"]').removeClass('hide');
			} else if (val == "Repaired at Region") {
				parentRow.find('select.rso_part_status option').addClass('hide');
				parentRow.find('select.rso_part_status option[value="Repaired at Region-Closed"]').removeClass('hide');
			} else if (val == "Sent to division to Analysis" ||
				val == "Sent to division to Repair" ||
				val == "Sent to Service Centre for Repair"
			) {
				parentRow.find('select.rso_part_status option[value=""]').attr("selected", "selected").trigger('change');
				parentRow.find('select.rso_part_status option').addClass('hide');
				parentRow.find('select.rso_part_status option[value="STO Awaited From Division"]').removeClass('hide');
				parentRow.find('select.rso_part_status option[value="Waiting for Shipment From Region"]').removeClass('hide');
				parentRow.find('select.rso_part_status option[value="Dispatched from region"]').removeClass('hide');
			} else if (val == "Sent to Vendor") {
				parentRow.find('select.rso_part_status option[value=""]').attr("selected", "selected").trigger('change');
				parentRow.find('select.rso_part_status option').addClass('hide');
				parentRow.find('select.rso_part_status option[value="Under assessment by Vendor"]').removeClass('hide');
				parentRow.find('select.rso_part_status option[value="Warranty Rejected by Vendor Closed"]').removeClass('hide');
				parentRow.find('select.rso_part_status option[value="New Parts Received under warranty by Vendor Closed"]').removeClass('hide');
				parentRow.find('select.rso_part_status option[value="Repaired Parts received under warranty by Vendor Closed"]').removeClass('hide');
				parentRow.find('select.rso_part_status option[value="Repaired on Paid basis by Vendor Closed"]').removeClass('hide');
				parentRow.find('select.rso_part_status option[value="Repaired on Prorata basis by Vendor Closed"]').removeClass('hide');
			} else {
				parentRow.find('select.rso_part_status option').addClass('hide');
			}
			let val1 = parentRow.find('select[data-extraname="rso_part_status"]').val();
			if (val1 == 'Waiting for Shipment From Region' || val == 'Dispatched from region') {
				parentRow.find('#sto_noDivCla').removeClass('hide');
				parentRow.find('.sto_no').removeClass('hide');
			} else {
				parentRow.find('#sto_noDivCla').addClass('hide');
	...
	dependencyWarrantyApplicable: function () {
		...
		$('select[data-extraname="rso_part_status"]').change(function () {
			let val = $(this).val();
			let parentRow = $(this).closest('tr');
			if (val == 'Waiting for Shipment From Region' || val == 'Dispatched from region') {
				parentRow.find('#sto_noDivCla').removeClass('hide');
				parentRow.find('.sto_no').removeClass('hide');
			} else {
				parentRow.find('#sto_noDivCla').addClass('hide');
	...
	registerBasicEvents: function (container) {
		...
		$('select.rso_part_status option').addClass('hide');
		...
		this.handleDependencyOfRepairedAndSentToRegion();
		this.handleDependencyAnalysisDone();
		$('select.action_taken_by_sm').on('change', function () {
			if ($(this).val() == "Scrapped at Region") {
				$(this).closest('tr').find('select.rso_part_status option').addClass('hide');
				$(this).closest('tr').find('select.rso_part_status option[value="Scrapped at Region-Closed"]').removeClass('hide');
			} else if ($(this).val() == "Sent to division to Analysis") {
				$(this).closest('tr').find('select.rso_part_status option[value=""]').attr("selected", "selected").trigger('change');
				$(this).closest('tr').find('select.rso_part_status option').addClass('hide');
				$(this).closest('tr').find('select.rso_part_status option[value="STO Awaited From Division"]').removeClass('hide');
				$(this).closest('tr').find('select.rso_part_status option[value="Waiting for Shipment From Region"]').removeClass('hide');
				$(this).closest('tr').find('select.rso_part_status option[value="Dispatched from region"]').removeClass('hide');
			} else if ($(this).val() == "Repaired at Region") {
				$(this).closest('tr').find('select.rso_part_status option').addClass('hide');
				$(this).closest('tr').find('select.rso_part_status option[value="Repaired at Region-Closed"]').removeClass('hide');
			} else if ($(this).val() == "Sent to division to Analysis" ||
				$(this).val() == "Sent to division to Repair" ||
				$(this).val() == "Sent to Service Centre for Repair"
			) {
				$(this).closest('tr').find('select.rso_part_status option[value=""]').attr("selected", "selected").trigger('change');
				$(this).closest('tr').find('select.rso_part_status option').addClass('hide');
				$(this).closest('tr').find('select.rso_part_status option[value="STO Awaited From Division"]').removeClass('hide');
				$(this).closest('tr').find('select.rso_part_status option[value="Waiting for Shipment From Region"]').removeClass('hide');
				$(this).closest('tr').find('select.rso_part_status option[value="Dispatched from region"]').removeClass('hide');
			} else if ($(this).val() == "Sent to Vendor") {
				$(this).closest('tr').find('select.rso_part_status option[value=""]').attr("selected", "selected").trigger('change');
				$(this).closest('tr').find('select.rso_part_status option').addClass('hide');
				$(this).closest('tr').find('select.rso_part_status option[value="Under assessment by Vendor"]').removeClass('hide');
				$(this).closest('tr').find('select.rso_part_status option[value="Warranty Rejected by Vendor Closed"]').removeClass('hide');
				$(this).closest('tr').find('select.rso_part_status option[value="New Parts Received under warranty by Vendor Closed"]').removeClass('hide');
				$(this).closest('tr').find('select.rso_part_status option[value="Repaired Parts received under warranty by Vendor Closed"]').removeClass('hide');
				$(this).closest('tr').find('select.rso_part_status option[value="Repaired on Paid basis by Vendor Closed"]').removeClass('hide');
				$(this).closest('tr').find('select.rso_part_status option[value="Repaired on Prorata basis by Vendor Closed"]').removeClass('hide');
			} else {
				$(this).closest('tr').find('select.rso_part_status option').addClass('hide');
			}
		});
		$('input[data-extraname="receivedvd_qty"]').on('input', function (event) {
			let recqty = $(this).closest('tr').find('input[data-extraname="sto_qty"]').val();
			$(this).closest('tr').find('.receivedvd_qty').attr('max', recqty);
		});
	},
	handleDependencyAnalysisDone : function () {
		let self = this;
		self.lineItemsHolder.find('tr.' + self.lineItemDetectingClass).each(function (index, domElement) {
			let parentRow = jQuery(domElement);
			let val = parentRow.find('input[data-extraname="ana_done_div_qty"]').val();
		...
		jQuery('input[data-extraname="ana_done_div_qty"]').on('input', function (event) {
			self.lineItemsHolder.find('tr.' + self.lineItemDetectingClass).each(function (index, domElement) {
				let parentRow = jQuery(domElement);
				let val = parentRow.find('input[data-extraname="ana_done_div_qty"]').val();
				if (val > 0) {
		...
		self.lineItemsHolder.find('tr.' + self.lineItemDetectingClass).each(function (index, domElement) {
			let parentRow = jQuery(domElement);
			let val = parentRow.find('input[data-extraname="asent_to_ven_qty"]').val();
			if (val > 0) {
				parentRow.find('#vendor_nameDivCla').removeClass('hide');
				parentRow.find('#vendor_responseDivCla').removeClass('hide');
			} else {
		...
		jQuery('input[data-extraname="asent_to_ven_qty"]').on('input', function (event) {
			self.lineItemsHolder.find('tr.' + self.lineItemDetectingClass).each(function (index, domElement) {
				let parentRow = jQuery(domElement);
				let val = parentRow.find('input[data-extraname="asent_to_ven_qty"]').val();
				if (val > 0) {
	...
	handleDependencyOfRepairedAndSentToRegion: function () {
		let self = this;
		self.lineItemsHolder.find('tr.' + self.lineItemDetectingClass).each(function (index, domElement) {
			let parentRow = jQuery(domElement);
			let val = parentRow.find('input[data-extraname="at_rep_a_sent_reg"]').val();
			if (val > 0) {
				parentRow.find('#sending_purposeDivCla').removeClass('hide');
				parentRow.find('#region_nameDivCla').removeClass('hide');
			} else {
				parentRow.find('#sending_purposeDivCla').addClass('hide');
				parentRow.find('#region_nameDivCla').addClass('hide');
			}
		});
		jQuery('input[data-extraname="at_rep_a_sent_reg"]').on('input', function (event) {
			self.lineItemsHolder.find('tr.' + self.lineItemDetectingClass).each(function (index, domElement) {
				let parentRow = jQuery(domElement);
				let val = parentRow.find('input[data-extraname="at_rep_a_sent_reg"]').val();
				if (val > 0) {
	...
	mapResultsToFields: function (parentRow, responseData) {
		...
		if (recordData.qty != undefined && recordData.qty != null) {
			...
			jQuery('select.rso_part_status option', parentRow).addClass('hide');
			// jQuery('.rso_part_status option[value="Return Sale Order Created"]', parentRow).attr("selected", "selected").trigger('change');
		}
	},
});