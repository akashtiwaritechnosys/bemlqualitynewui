	...
	public function getAllLineItemForParent($parentId){
		...
		if($result){
			...
			for ($i = 0 ; $i < $rowCount ; ++$i) {
				...
				if($IGMODULE == 'FailedParts' || $IGMODULE == 'ServiceOrders' ||  $IGMODULE == 'ReturnSaleOrders' ||  $IGMODULE == 'ReturnSaleOrder' || $IGMODULE == 'SalesOrder'){
					$elementNew = [];
					foreach($element as $key => $value){
						if (in_array($key, $viewableFields)) {
							$elementNew[$key] = $value;
						}
						if ($key == 'line_vendor_id') {
						...
						} else if ($key == 'at_rep_a_sent_reg') {
							$valArr = json_decode($value);
							if (empty($valArr)) {
								$elementNew[$key] = [];
							} else {
								$elementNew[$key] = $valArr;
							}
						}
					}
