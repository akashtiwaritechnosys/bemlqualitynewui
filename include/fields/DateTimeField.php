...
class DateTimeField {
	...
	public function getDisplayDateTimeValueMobile($user = null) {
		return $this->getDisplayDate($user) . ' ' . $this->getDisplayTimeMobile($user);
	}
	...
	function getDisplayTimeMobile( $user = null ) {
		$date = self::convertToUserTimeZone($this->datetime, $user);
		$time = $date->format("H:i:s");
