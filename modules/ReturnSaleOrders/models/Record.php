...
class ReturnSaleOrders_Record_Model extends Inventory_Record_Model {
    ...
    function getAssociatedProducts($module, $focus, $seid = '', $refModuleName = false) {
        ...
        if (in_array($module, $inventoryModules)) {
            ...
            if (($data['office'] == 'Production Division')) {
					...
					WHERE vtiger_inventoryproductrel.id=? and vtiger_inventoryproductrel.div_or_ser_center = ? and vtiger_inventoryproductrel.rso_part_status = 'Dispatched from region' ORDER BY sequence_no";
                ...
                $query = "SELECT
					...
					LEFT JOIN vtiger_products ON vtiger_products.productid=vtiger_inventoryproductrel.productid
					LEFT JOIN vtiger_service ON vtiger_service.serviceid=vtiger_inventoryproductrel.productid
					WHERE vtiger_inventoryproductrel.id=? and vtiger_inventoryproductrel.rso_part_status != 'Dispatched from region'  ORDER BY sequence_no";
    ...
    function getAssociatedProductsDetail($module, $focus, $seid = '', $refModuleName = false) {
        global $log;
        $log->debug("Entering getAssociatedProducts(" . $module . "," . get_class($focus) . "," . $seid . "='') method ...");
        global $adb;
        $output = '';
    ...
    function getProductsDetail() {
        $numOfCurrencyDecimalPlaces = getCurrencyDecimalPlaces();
        $relatedProducts = $this->getAssociatedProductsDetail($this->getModuleName(), $this->getEntity());
        $productsCount = count($relatedProducts);
