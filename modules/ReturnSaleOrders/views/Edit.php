...
class ReturnSaleOrders_Edit_View extends Inventory_Edit_View {
    ...
    public function process(Vtiger_Request $request) {
        ...
        if ($request->has('totalProductCount')) {
            ...
            $parentLineItemId = $request->get('parent_line_itemid');
            $viewer->assign('RSO_CREATABLE_QTY', $this->getRSOCReatableQty($referenceId, $parentLineItemId));
            ...
            $typeOfLineItem = $relatedProducts[1]['validated_part_no1'];
            $typeOfLineItem = substr($typeOfLineItem, -3);
            $viewer->assign('typeOfLineItem', $typeOfLineItem);
            $recordModel = Vtiger_Record_Model::getCleanInstance($moduleName);
            ...
            $referenceId = $request->get('sale_order_id');
            $parentLineItemId = $request->get('parent_line_itemid');
            $viewer->assign('RSO_CREATABLE_QTY', $this->getRSOCReatableQtyWhileEdit($record));
        ...
        $dependentList = array(
            ...
            'div_or_ser_center', 'sending_purpose', 'region_name', 'analysis_done_doc'
        );
        $pickListFields = [];
        while ($row = $adb->fetch_array($result)) {
            ...
            if ($row['fieldname'] == 'sto_no') {
                ...
                $funcName = array('name' => 'maxsizeInSignUP', 'params' => '10');
				array_push($validator, $funcName);
        ...
        $dependentDopDownValuesOfActionFRR = [
            'Repaired at Region' => 'Repaired at Region',
            'Sent to division to Analysis' => 'Sent to division to Analysis',
            'Sent to division to Repair' => 'Sent to division to Repair',
            'Sent to Service Centre for Repair' => 'Sent to Service Centre for Repair',
            'Sent to Vendor' => 'Sent to Vendor'
        ];
        $viewer->assign('dependentDopDownValuesOfActionFRR', $dependentDopDownValuesOfActionFRR);

        $dependentDopDownValuesOfActionFRN = [
            'Scrapped at Region' => 'Scrapped at Region',
            'Sent to division to Analysis' => 'Sent to division to Analysis'
        ];
        $viewer->assign('dependentDopDownValuesOfActionFRN', $dependentDopDownValuesOfActionFRN);
        ...
        $viewer->assign('cust_role', $data['cust_role']);
        $viewer->assign('office', $data['office']);
        if ($data['cust_role'] == 'Service Manager' && $data['office'] != 'Service Centre') {
    ...
    public function getRSOCReatableQtyWhileEdit($refeRenceId) {
        global $adb;
        $sql =  'SELECT quantity FROM `vtiger_inventoryproductrel` 
                where id = ?';
        ...
        $sql =  'SELECT sto_qty FROM `vtiger_inventoryproductrel` 
                where id = ? and rso_part_status = "Dispatched from region"';
        $result = $adb->pquery($sql, array($refeRenceId));
        $finalQtyD = 0;
        while ($row = $adb->fetch_array($result)) {
            $finalQtyD = $finalQtyD + floatval($row['sto_qty']);
        }

        return floatval($finalQty) - floatval($finalQtyD);
    }

    public function getRSOCReatableQty($refeRenceId, $parentLineItemId) {
        global $adb;
        $sql =  'SELECT final_qty FROM `vtiger_inventoryproductrel` 
                where id = ? and lineitem_id = ?';
        $result = $adb->pquery($sql, array($refeRenceId, $parentLineItemId));
        $finalQty = 0;
        while ($row = $adb->fetch_array($result)) {
