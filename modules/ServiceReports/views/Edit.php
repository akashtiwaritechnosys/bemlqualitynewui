...
class ServiceReports_Edit_View extends Inventory_Edit_View {
	...
	public function process(Vtiger_Request $request) {
		...
		$newNonEdit = [];
		if (
			$ticketType == 'ERECTION AND COMMISSIONING' || $ticketType == 'PRE-DELIVERY'
			|| $ticketType == 'INSTALLATION OF SUB ASSEMBLY FITMENT'
		) {
			foreach ($nonEditKeys as $key => $val) {
				if ($val == 'account_id') {
					continue;
				}
				array_push($newNonEdit, $val);
			}
			$viewer->assign('NONEDITABLEKEYS', $newNonEdit);
		} else {
		...
		while ($row = $adb->fetch_array($result)) {
			...
			if ($row['uitype'] == '16') {
				...
				if ($row['fieldname'] == 'sr_action_one' && $ticketType == 'GENERAL INSPECTION') {
					$arr = [];
					$arr['To be replaced'] = 'To be replaced';
					$arr['To be Repaired'] = 'To be Repaired';
					$row['picklistValues'] = $arr;
				}
			}
		...
		$validator = [
			array('name' => 'lessThanOrEqualToToday')
		];
		$viewer->assign('specialValidator', $validator);
