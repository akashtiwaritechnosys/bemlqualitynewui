...
class ServiceReports_FilePreview_View extends Vtiger_IndexAjax_View {
	...
	public function process(Vtiger_Request $request) {
		...
		$imageFileTypes = array('image/gif', 'image/png', 'image/jpeg', 'jpg', 'png', 'jpeg', 'gif', 'pjpeg', 'svg');
		//supported by video js
		$videoFileTypes = array('video/mp4', 'video/ogg', 'audio/ogg', 'video/webm', 'mp4', 'ogg', 'webm');
		...
		} else if (in_array($extn, $videoFileTypes)) {
			$viewer->assign('VIDEO_FILE_TYPE', 'yes');
		} else if (in_array($extn, $imageFileTypes)) {
			$viewer->assign('IMAGE_FILE_TYPE', 'yes');
		} else {
