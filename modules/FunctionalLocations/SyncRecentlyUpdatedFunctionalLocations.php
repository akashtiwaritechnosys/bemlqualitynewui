<?php
function SyncRecentlyUpdatedFunctionalLocations() {
	global $adb;
	...
	foreach ($xml as $key => $value) {
		...
		if ($num_rows > 0) {
			...
			$recordModel->set('assigned_user_id', thisgetAssignedUserOfTheFunctionalLocation($sapRefNum));
			...
			$focus->column_fields['assigned_user_id'] = thisgetAssignedUserOfTheFunctionalLocation($sapRefNum);
			$focus->save("FunctionalLocations");
		}
	}
}

function thisgetAssignedUserOfTheFunctionalLocation($functionalName) {
	global $adb;
