...
function EquipmentAvailabilityCalculation($entityData) {
    ...
    $starttimestamp = strtotime($recordInfo['start_date']);
	$endtimestamp = strtotime($recordInfo['end_date']);
	$numberOfDays = abs($endtimestamp - $starttimestamp)/86400;
    ...
    $totalHoursInMonth = $shiftHours * ($numberOfDays + 1);
    ...
    $systemTotalBreakDownHours = (float)$recordInfo['system_total_breakdown'];
    $eqAvaSystem = (($totalHoursInMonth - $systemTotalBreakDownHours - $consideredMaint) / $totalHoursInMonth) * 100;
    $eqAvaSystem = number_format($eqAvaSystem, 2, '.', '');
