...
function EquipmentAvailabilityCalculateIPeriodic($entityData) {
	...
	$resultOfAllEQ = $db->pquery($sqlForGettingAll, array('Applicable'));
	while ($rowDataOfParent = $db->fetchByAssoc($resultOfAllEQ)) {
		...
		if ($commitedAvailability == 'Different availability applicable during contract period') {
			...
			LEFT JOIN vtiger_inventoryproductrel_equipment 
