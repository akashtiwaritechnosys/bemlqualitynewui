...
function getJoinInformation($params) {
    ...
    if (!empty($params['old_oth_as'])) {
        $old_oth_as = $params['old_oth_as'];
    }
    if (!empty($params['old_oth_fieldid'])) {
        $old_fieldid = '_' . $params['old_oth_fieldid'];
    }
    if (isset($params['join_type']) && 'MIF' === $params['join_type']) {
        if (!empty($params['fieldtabid'])) {
            $mif_alias .= '_' . $params['fieldtabid'];
        }
    }
    // ITS4YOU-UP SlOl 12/14/2013 6:09:39 PM vtiger_cntactivityrel UPDATE
    if ('vtiger_cntactivityrel' === $params['tablename']) {
    ...
    } elseif (isset($params['join_type']) && 'MIF' === $params['join_type'] && empty($using_module_arr)) {
    ...
    } elseif ('vtiger_crmentity' !== $params['tablename'] || in_array($params['field_uitype'], array('52', '53', '531'))) {
        $adb = PEARDatabase::getInstance();
        $sp_join_res = $adb->pquery('SELECT uitype,tablename,columnname  FROM vtiger_field WHERE fieldid = ?', array($params['fieldid']));
        $sp_join_row = $adb->fetchByAssoc($sp_join_res, 0);
...
// show("UIP N1");
        ...
        } elseif ('vtiger_inventoryproductrel' === $params['tablename']) {
...
// show("UIP N2");
            ...
            if (vtlib_isModuleActive($gmodule)) {
                ...
                if (is_array($params['table_index'])) {
                    if (isset($ui_focus->table_name) && !empty($ui_focus->tab_name_index[$ui_focus->table_name]) && $params['tablename'] != $ui_focus->table_name) {
// show("UIP N3");
