...
class UIType10 extends UITypes {
    ...
    public function getJoinSQL_UI10(&$join_array, &$columns_array, $uirelmodule) {
        ...
        if (vtlib_isModuleActive($uirelmodule)) {
            ...
            if(array_key_exists($this->params['tablename'],$this->params['table_index'])
                && $this->params['tablename']!=$this->params['report_primary_table']
            ){
                ...
                if (!array_key_exists(" $cf_table_name AS $cf_table_name" . $cf_fieldid_alias . ' ', $join_array)
            ...
            // ITS4YOU-END
            if(!array_key_exists(" $uirel_table_name AS $uirel_table_name" . $fieldid_alias . ' ', $join_array)
...
// show("oldo ui 10 joining"," $uirel_table_name AS $uirel_table_name".$fieldid_alias." ",$join_array,"oldo ui 10 joining post");
    ...
    public function getJoinSQL(&$join_array, &$columns_array) {
        ...
        if (!empty($fieldid_alias_row) && array_key_exists('fieldid', $fieldid_alias_row)) {
        ...
        if('vtiger_modcomments' === $this->params['tablename']
            && in_array($this->params['columnname'], ['userid', 'customer',])
        ){
...
// show("<font color='green'>fielduitype10_IN_JO_".$field_uitype,$using_array,$this->params,$join_array,"fielduitype10_IN_JO_PO","</font>");
        ...
        if(in_array($this->params['columnname'], ITS4YouReports::$intentory_fields)
            && (false === strpos($site_URL, 'ateccrm') && 361 !== intval($this->params['report_id']))
        ){
...
// show("oldo ui 10 intentory join000",$join_array,"oldo ui 10 intentory join000 POST");
        ...
        if (!empty($this->params['columnname'])) {
            ...
            if(!empty($using_tablename) && !empty($using_columnname)){
