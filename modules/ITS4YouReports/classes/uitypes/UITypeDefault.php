...
use ITS4YouReports_GenerateObjUtils_Helper as GUtils;
...
class UITypeDefault extends UITypes {
    ...
    public function getJoinSQL(&$join_array, &$columns_array) {
        ...
        if('vtiger_modtracker_detail' === $this->params['tablename']
            || 'vtiger_modtracker_basic' === $this->params['tablename']
        ){
        ...
        }elseif (!array_key_exists($join_alias,$join_array)
            && $this->params['primary_table_name']!==$join_tablename_alias
            && isset($this->params['using_array'])
            && !empty($this->params['using_array']['using'])
            && $clear_tablename !== 'vtiger_crmentity'
        ) {
            ...
            if ($join_tablename != $this->params['primary_table_name'] && !empty($clearUsingTableName)) {
                ...
                if (!array_key_exists($clearUsingTableJoinAlias, $join_array) && array_key_exists($clearUsingTableName, $this->params['table_index'])) {
                    ...
                    if ('vtiger_user2role' === $this->params['tablename']) {
                        $join_array[$clearUsingTableJoinAlias]['using'] = 'vtiger_crmentity.smownerid';
                    } else {
        ...
        if (!empty($this->params['fld_string'])) {
        ...
        if ('vtiger_modtracker_detail' === $this->params['tablename']
        ...
        } elseif ('vtiger_activity' === $this->params['tablename'] && (in_array($this->params['columnname'], ['time_start', 'time_end']))) {
            if (array_key_exists(' ' . $join_tablename_alias . '.' . $this->params['columnname'] . ' ', $columns_array)) {
                return '';
            }
            $fld_cond = ' ' . $join_tablename_alias . '.' . $this->params['columnname'] . ' ';
            $columns_array_value = $fld_cond . ' AS ' . $this->params['columnname'] . $old_oth_as . $old_oth_fieldid . $mif_as;
        }elseif ('vtiger_activity' === $this->params['tablename'] && (in_array($this->params['columnname'], array('date_start')))) {
        ...
        }elseif ('vtiger_activity' === $this->params['tablename']
            && (in_array($this->params['columnname'], array('end_date', 'due_date')))
            && GUtils::isDTType($this->params['fld_string'])
        ) {
            ...
            $columns_array_value = $fld_cond . ' AS end_date' . $old_oth_as . $old_oth_fieldid . $mif_as;
        }elseif ('vtiger_activity' === $this->params['tablename']
            && (in_array($this->params['columnname'], array('due_date')))
        ) {
            if (array_key_exists(" $join_tablename_alias.due_date ", $columns_array))
                return '';
            $fld_cond = " $join_tablename_alias.due_date ";
            $columns_array_value = $fld_cond . ' AS ' . $this->params['columnname'] . $old_oth_as . $old_oth_fieldid . $mif_as;

        }elseif('DT' === $typeofdata){
