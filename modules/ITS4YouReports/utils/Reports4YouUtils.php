...
function getPrimaryColumns_GroupingHTML($module, $selected = "", $ogReport = "") {
    ...
    if (vtlib_isModuleActive($module)) {
        ...
        foreach ($ogReport->module_list[$module] as $key => $value) {
            if (isset($ogReport->pri_module_columnslist[$module][$value])
                && !array_key_exists($value, $block_listed)
            ) {
        ...
        for ($i = 0; $i < ITS4YouReports_Functions_Helper::count($secmodule); $i++) {
            ...
            if (vtlib_isModuleActive($modulename)) {
                ...
                foreach ($ogReport->module_list[$modulename] as $key => $value) {
                    if (isset($ogReport->sec_module_columnslist[$modulename][$value])
                        && !array_key_exists($value, $block_listed)
                    ) {
                        $block_listed[$value] = true;
...
function sgetNewColumnstoTotalHTMLScript($Objects) {
    ...
    foreach ($Objects as $key => $ObjectRow) {
        ...
        $last_key = ITS4YouReports_Functions_Helper::count($ObjectRow_array) - 1;
        ...
        if ($clear_tablename != "") {
            ...
            if (in_array($sc_columnname, ['ps_producttotal', 'ps_productstotalafterdiscount', 'ps_productvatsum', 'ps_producttotalvatsum', 'ps_producttotalsum',])
        ...
        foreach ($ogReport->pri_module_columnslist[$module] as $block_key => $field_array) {
            ...
            foreach ($cal_options as $b => $inter) {
                ...
                if ($block_key != "Custom Information") {
                    if (!empty($intersect)) {
                        if (!is_array($cal_options[$block_key])) {
                            $cal_options[$block_key] = [];
                        }
        ...
        for ($i = 0; $i < ITS4YouReports_Functions_Helper::count($secmodule); $i++) {
            ...
            if (vtlib_isModuleActive($modulename)) {
                ...
                foreach ($ogReport->module_list[$modulename] as $key => $value) {
                    ...
                    if (array_key_exists(trim($modulename . ' ' . trim($fieldname)), $ogReport->sec_module_columnslist)
                        && isset($ogReport->sec_module_columnslist[trim($modulename . ' ' . trim($fieldname))][$value])
                        && !array_key_exists($value, $block_listed)
                    ) {
                        $block_listed[$value] = true;
                ...
                if (array_key_exists('primarymoduleid', $_REQUEST) && array_key_exists('selectedmodule', $_REQUEST)
                    && $_REQUEST['primarymoduleid'] !== $_REQUEST['selectedmodule']
                    && $access_count_listed !== true
                    && in_array($modulename, array('Calendar',))
                ) {
...
function sgetColumntoTotalOptions($Options, $primarymodule, $secondarymodules) {
    ...
    if (!empty($secondarymodules)) {
        ...
        for ($i = 0; $i < ITS4YouReports_Functions_Helper::count($secondarymodules); $i++) {
...
function getR4UStdCriteriaByModule($module) {
    ...
    if ($r4u_stdcriteria_name != "" && isset($_SESSION[$r4u_stdcriteria_name]) && !empty($_SESSION[$r4u_stdcriteria_name])) {
        ...
        foreach ($module_info[$module] as $key => $blockid) {
            ...
            if (1 < ITS4YouReports_Functions_Helper::count($multiCheck)) {
