...
class ITS4YouReports_KeyMetrics_Model extends Vtiger_Base_Model {
    ...
    function getReports($pagingModel,$search_params=array()) {
        ...
        /*
          ...
          $reportsCount += ITS4YouReports_Functions_Helper::count($reports);
          }
          }else{
          $reportsCount = ITS4YouReports_Functions_Helper::count($reportsList);
          }
         */
        ...
        if ($fldrId == false) {
            ...
            $reportModels = array();
            for ($i = 0; $i < ITS4YouReports_Functions_Helper::count($reportsList); $i++) {
    ...
    public function getRecordIds($skipRecords = false, $module) {
        ...
        if ($skipRecords && !empty($skipRecords) && is_array($skipRecords) && ITS4YouReports_Functions_Helper::count($skipRecords) > 0) {
