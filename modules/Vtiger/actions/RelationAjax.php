...
class Vtiger_RelationAjax_Action extends Vtiger_Action_Controller {
	...
	function addRelation($request) {
		...
		if ($relatedRecordIdList != 'all') {
			...
			} else if ($sourceModule == 'Contacts' && $relatedModule == 'FunctionalLocations') {
				require_once('include/utils/GeneralUtils.php');
				foreach ($relatedRecordIdList as $relatedRecordId) {
					$checkpresence = $adb->pquery("SELECT crmid FROM vtiger_crmentityrel WHERE
					crmid = ? AND module = ? AND relcrmid = ? AND relmodule = ?", array($sourceRecordId, 'Contacts', $relatedRecordId, 'FunctionalLocations'));
					if ($checkpresence && $adb->num_rows($checkpresence))
						continue;
					$adb->pquery("INSERT INTO vtiger_crmentityrel(crmid, module, relcrmid, relmodule) VALUES(?,?,?,?)", array($sourceRecordId, 'Contacts', $relatedRecordId, 'FunctionalLocations'));

					$anotherRelations = getAllAssociatedEquipmentsBasedSELInkedInEmployeeFunc($relatedRecordId);
					foreach ($anotherRelations as $anotherRelation) {
						// $eqRelationModel->addRelation($sourceRecordId,$anotherRelation);
						$checkpresence = $adb->pquery("SELECT crmid FROM vtiger_crmentityrel WHERE
														crmid = ? AND module = ? AND relcrmid = ? AND relmodule = ?", array($sourceRecordId, 'Contacts', $anotherRelation, 'Equipment'));
						if ($checkpresence && $adb->num_rows($checkpresence))
							continue;
						$adb->pquery("INSERT INTO vtiger_crmentityrel(crmid, module, relcrmid, relmodule) VALUES(?,?,?,?)", array($sourceRecordId, 'Contacts', $anotherRelation, 'Equipment'));
					}
				}
			} else {
				foreach($relatedRecordIdList as $relatedRecordId) {
