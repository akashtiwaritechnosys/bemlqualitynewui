				...
				try {
					if (vtws_recordExists($recordId)) {
						...
						if (isset($this->recordValues['id'])) {
							...
							if ($module == 'HelpDesk') {
								global $adb;
								$recordCreator = explode('x', $contactId);
								$recordCreator = $recordCreator[1];
								$sql = 'UPDATE `vtiger_crmentity` SET `smcreatorid` = ? WHERE `vtiger_crmentity`.`crmid` = ?';
								$idComponents = explode('x', $this->recordValues['id']);
								$recordId = $idComponents[1];
								$adb->pquery($sql, array($recordCreator, $recordId));
							}
						}
						...
						// 	$focus->uploadAndSaveFile($recordId, $module, $_FILES['file'], 'Attachment', 'imagename');
						// }
