...
abstract class CustomerPortal_API_Abstract {
	...
	function getAccessiblePlatforms($userName) {
		global $adb;
		$sql = 'select con_approval_status from vtiger_contactdetails
		INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_contactdetails.contactid
		where contact_no = ? and vtiger_crmentity.deleted = 0';
	...
	function authenticatePortalUser($username, $password) {
		...
		if (empty($loginPlatDEtails)) {
			throw new Exception('You Are Not Registered', 0);
		}
		if ($loginPlatDEtails['con_approval_status'] !== 'Accepted' && $loginPlatDEtails['con_approval_status'] !== 'Rejected') {
			throw new Exception('User Verfication Is Pending', 0);
		}
		if ($loginPlatDEtails['con_approval_status'] == 'Rejected') {
			throw new Exception('User Verfication Is Rejected , Please Sign Up Again', 0);
		}
		global $adb;
		...
		$sql = "SELECT id, user_name,vtiger_contactdetails.usr_log_plat,
		vtiger_contactdetails.req_for_del,
				vtiger_portalinfo.user_password,last_login_time, isactive,
				support_start_date, support_end_date, cryptmode FROM vtiger_portalinfo
			...
			ON vtiger_contactdetails.contact_no=vtiger_portalinfo.user_name
		...
		if ($num_rows >= 0) {
			...
			if ($passwordNotMatching == true) {
				throw new Exception('Password Is Incorrect', 0);
			}
			...
			if ($customerId) {
				...
				if ($isActive && ($support_end_date >= $current_date || $support_end_date == null)) {
					...
					$isProfileDeleteRequested = $adb->query_result($result, $i, 'req_for_del');
					if($isProfileDeleteRequested == '1'){
						throw new Exception("Profile Delete Is Requested", 0);
					}
