	...
	function process(CustomerPortal_API_Request $request) {
		...
		if ($current_user) {
			...
			if($module == 'Documents') {
				...
				global $site_URL_Another;
				foreach ($result as $key => $recordValues) {
					$idN = $recordValues['id'];
					$idN = explode('x', $idN);
					$idN = $idN[1];
					$result[$key]['doc_url'] = $site_URL_Another . 'modules/Mobile/v2/DownloadFile?record='.$idN.'&access_token='.$request->get('access_token').'&useruniqueid='.$request->get('useruniqueid');
					$result[$key]["starred"] = "";
					$result[$key]['smownerid'] = $recordValues['assigned_user_id'];
					unset($result[$key]['assigned_user_id']);
					unset($result[$key]['documentExists']);
					$result[$key]['modifiedby_idofreference'] = "19x1";
					$result[$key]['modifiedby_Label'] = "";
					$result[$key]['modifiedtime'] = Vtiger_Datetime_UIType::getDisplayDateTimeValueMobile($recordValues['modifiedtime']);
					$result[$key]['createdtime'] = Vtiger_Datetime_UIType::getDisplayDateTimeValueMobile($recordValues['createdtime']);
				}
			}
