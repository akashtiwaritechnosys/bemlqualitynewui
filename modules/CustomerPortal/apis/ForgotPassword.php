	...
	function process(CustomerPortal_API_Request $request) {
		...
		$sql = 'SELECT * FROM vtiger_portalinfo
					...
					INNER JOIN vtiger_crmentity ON vtiger_portalinfo.id=vtiger_crmentity.crmid 
						WHERE vtiger_portalinfo.user_name = ? AND vtiger_contactdetails.email = ? AND 
						vtiger_crmentity.deleted= ?';

		$res = $adb->pquery($sql, array($username, $mailid, '0'));
		...
		if ($num_rows > 0) {
			...
			$response->setError('1412', 'Email Associated With UserName Is Not Matching');
		}
		return $response;
	}
