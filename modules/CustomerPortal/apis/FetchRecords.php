				...
				if (!empty($fieldsArray)) {
					...
					foreach ($fieldsArray as $key => $value) {
						if (strpos($value[2], ',') !== false) {
							$a = explode(',', $value[2]);
							$c = "'". implode("','", $a);
							$countSql .= $value[0] . ' IN(' .$c . "') " . $groupConditionsBy . " ";
							$sql .= $value[0] . ' IN(' . $c . "') " . $groupConditionsBy . " ";
						} else {
							$countSql .= $value[0] . ' LIKE \'%' . $value[2] . "%' " . $groupConditionsBy . " ";
							$sql .= $value[0] . ' LIKE \'%' . $value[2] . "%' " . $groupConditionsBy . " ";
						}
					}
					$countSql = CustomerPortal_Utils::str_replace_last($groupConditionsBy, '', $countSql);
					$sql = CustomerPortal_Utils::str_replace_last($groupConditionsBy, '', $sql);
