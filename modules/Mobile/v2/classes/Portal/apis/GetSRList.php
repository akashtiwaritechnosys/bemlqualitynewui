...
class Portal_GetSRList_API extends Portal_Default_API {
	...
	public function process(Portal_Request $request) {
		...
		if (!empty($headerFieldStatusValue) && empty($headerFieldPurposeValue)) {
			array_push($AdditionalConditions, array('ticket_type', 'e' , $headerFieldStatusValue));
		} else if (!empty($headerFieldStatusValue) && !empty($headerFieldPurposeValue)) {
			array_push($AdditionalConditions, array('ticket_type', 'e' ,$headerFieldStatusValue));
			array_push($AdditionalConditions, array('purpose' , 'e', $headerFieldPurposeValue));
		}
		...
		if (empty($pageLimit))
			$pageLimit = 10;
		$filter = $request->get('search_params');
