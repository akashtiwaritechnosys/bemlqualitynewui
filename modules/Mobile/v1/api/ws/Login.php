...
class Mobile_WS_Login extends Mobile_WS_Controller {
	...
	function process(Mobile_API_Request $request) {
		...
		if ($loginPlatDEtails['req_for_del'] == '1') {
			$response->setError(1501, 'Profile Delete Is Requested');
			return $response;
		}
		if (!$current_user->doLogin($password)) {
			...
			$result = array(
				...
				'usertype' => "BEMLUSER",
				'employeeWsId' => Mobile_WS_Utils::getEntityModuleWSId('ServiceEngineer'). 'x' . $data['serviceengineerid'],
				'access_token' => $jwt,
				'username' => $username,
				'password' => $password,
	...
	function getAccessiblePlatforms($userName) {
		global $adb;
		$sql = 'select ser_usr_log_plat,badge_no,approval_status,req_for_del from vtiger_serviceengineer
		INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_serviceengineer.serviceengineerid
		where badge_no = ? ';
