...
class HelpDesk_GetAllInfo_Action extends Vtiger_IndexAjax_View {
	...
	public function process(Vtiger_Request $request) {
		...
		$runningStatus = $recordModel->get('eq_run_war_st');
		$orgId = $recordModel->get('account_id');
		...
		if ($runningStatus == 'Outside Warranty') {
			$data['warranty_message'] = 'Equipment is Outside Warranty, Customer is ' . $chargableOutSideWarrntyText;
		} else {
			$data['warranty_message'] = '';
		}
