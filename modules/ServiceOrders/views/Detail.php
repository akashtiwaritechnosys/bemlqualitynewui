...
class ServiceOrders_Detail_View extends Inventory_Detail_View {
    ...
    public function showModuleDetailView(Vtiger_Request $request) {
        ...
        $data = $this->configuredLineItemFieldsWithOutDependWithoutNon($moduleName);
    ...
    function configuredLineItemFieldsWithOutDependWithoutNon($moduleName) {
        global $adb;
        $tabId = getTabId($moduleName);
        $sql = "SELECT * FROM `vtiger_field` LEFT JOIN vtiger_blocks
             on vtiger_blocks.blockid = vtiger_field.block where vtiger_field.tabid = ? 
        ...
        while ($row = $adb->fetch_array($result)) {
            if($row['fieldname'] == 'collect_immidiately'){
                continue;
            }
            if ($row['uitype'] == '16') {
                $row['picklistValues'] = getAllPickListValues($row['fieldname']);
