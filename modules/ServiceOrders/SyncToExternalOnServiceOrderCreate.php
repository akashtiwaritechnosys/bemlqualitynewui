...
function getProductsOfSTO($recordId) {
    ...
    for ($i = 0; $i < $num_rows; $i++) {
        $product = array();
        $productNmeToSyncToSAP = $adb->query_result($result, $i, 'productname');
        if(empty($productNmeToSyncToSAP)){
            $product['MATNR'] = $adb->query_result($result, $i, 'manual_part_no_so');
        } else {
            $product['MATNR'] = $productNmeToSyncToSAP;
        }
        $product['MENGE'] = $adb->query_result($result, $i, 'quantity');
        $product['POSTP'] = 'L';
        $product['REMARKS'] = $adb->query_result($result, $i, 'lid_remarks');
