...
function ExternalAppSyncUpdated($entityData) {
    ...
    foreach ($xml['IT_SERVORDER'] as $key => $value) {
        $sql = "insert into vtiger_temp_so_table(soid,qty,part_number,lid_remarks) values (?,?,?,?)";
        $adb->pquery(
            $sql,
            array(
                trim($value['AUFNR']),
...
function updateLineItemsSO($id) {
    ...
    while ($row = $adb->fetch_array($result)) {
        $insertSql = "insert into vtiger_inventoryproductrel(id,productid,productname,quantity,comment,lid_remarks)
         values (?,?,?,?,?,?)";
