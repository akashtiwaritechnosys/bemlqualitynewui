...
class Settings_LabelManager_SaveLanguage_Action extends Vtiger_IndexAjax_View {
	...
	function __construct() {
		...
		$this->exposeMethod('saveNewLabel');
	}
	...
	public function saveNewLabel(Vtiger_Request $request) {
		global $root_directory;
		$viewer = $this->getViewer($request);
		$qualifiedModuleName = $request->getModule(false);
		...
		$serializeData = explode('&', urldecode($request->get('serializeData')));
		$language = $request->get('language');
		$labelKey = $request->get('labelKey');
		$labelValue = $request->get('labelValue');
        ...
        foreach ($languageFileArray['languageStrings'] as $languageStringsKey => $languageStringsValue) {
            $line = str_replace("'", "\'", stripslashes($languageStringsValue));
		...
		$newLanguage .= "\t" . "'" . $labelKey . "' => '" . $labelValue . "'," . PHP_EOL;
        $newLanguage .= ");" . PHP_EOL . PHP_EOL;
        ...
        $loadUrl = 'index.php?module=LabelManager&parent=Settings&view=LabelManager&mode=languageManage&sourceModule='.$sourceModule.'&language='.$language;
