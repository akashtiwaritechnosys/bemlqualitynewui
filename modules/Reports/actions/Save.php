...
class Reports_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		global $adb;
		if($record){
			$adb->pquery("DELETE FROM vtiger_pivot_report_fields WHERE reportid = ?", array($record));
		}
		$query = $adb->pquery("INSERT INTO vtiger_pivot_report_fields(reportid, pivot_columns, pivot_rows, pivot_datafield) VALUES (?,?,?,?)", array($reportModel->getId(), $pivot_rows, $pivot_columns, $pivot_datafield));

		if($type == 'Pivot'){
			$adb->pquery("UPDATE vtiger_report SET report_type = ? WHERE reportid = ?", array($type, $reportModel->getId()));
		}
		//Pivot Report
