...
class Reports_Record_Model extends Vtiger_Record_Model {
	...
	function getEditViewUrl() {
		...
		global $adb;
		$query = $adb->pquery("SELECT * FROM vtiger_report WHERE reportid = ?", array($this->getId()));
		$report_type = $adb->query_result($query, 0, 'report_type');
		if($report_type == 'Pivot'){
			return 'index.php?module='.$this->getModuleName().'&view='.$view.'&record='.$this->getId().'&type=Pivot';
		}else{
			return 'index.php?module='.$this->getModuleName().'&view='.$view.'&record='.$this->getId();
		}
	}
