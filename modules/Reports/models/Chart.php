...
class Reports_Chart_Model extends Vtiger_Base_Model {
	...
	//Pivot Report
	function getGroupByField1() {
		return $this->get('groupbyfield1');
	}
	//Pivot Report
...
abstract class Base_Chart extends Vtiger_Base_Model{
	...
	function getQuery() {
		...
		$groupByFiledInfo1 = explode(':', $this->getParent()->getGroupByField1());//Pivot Report
		...
		//Pivot Report
		global $adb;
		$query = $adb->pquery("SELECT * FROM vtiger_report WHERE reportid = ?", array($_REQUEST['record']));
		$report_type = $adb->query_result($query, 0, 'report_type');

		if($report_type == 'Pivot'){
			$chartSQL .= ", ".$groupByFiledInfo1[0].".".$groupByFiledInfo1[1]." AS ".$groupByFiledInfo1[2];
		}
		//Pivot Report
		...
		if($report_type == 'Pivot'){
			if($groupByColumns && is_array($groupByColumns)) {
				$chartSQL .= " GROUP BY " . implode(',', $groupByColumns).", ".$groupByFiledInfo1[0].".".$groupByFiledInfo1[1];
			}
		}else{
			if($groupByColumns && is_array($groupByColumns)) {
...
class VerticalbarChart extends Base_Chart {
	function generateData() {
		...
		$groupByFiledInfo1 = $this->getParent()->getGroupByField1();//Pivot Report
