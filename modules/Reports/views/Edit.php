...
Class Reports_Edit_View extends Vtiger_Edit_View {
	...
	function step2(Vtiger_request $request) {
		...
		//Pivot Report
		$operationValue = array('SUM:2','AVG:3','MIN:4','MAX:5');

		$operationFieldValues = array();
		foreach ($operationValue as $key => $value) {
			foreach ($reportModel->getCalculationFields() as $key1 => $value1) {
				foreach ($value1 as $key2 => $value2) {
					$operationFieldValues[$key2.$value] = $value2.'-'.$value;
				}
			}
		}

		$viewer->assign('operationFieldValues', $operationFieldValues); 
		$viewer->assign('type', $type); 
		global $adb;
		$pivotDataQuery = $adb->pquery("SELECT * FROM vtiger_pivot_report_fields WHERE reportid = ?", array($record));
		$pivot_columns = explode(',', $adb->query_result($pivotDataQuery, 0, 'pivot_columns'));
		$pivot_rows = explode(',', $adb->query_result($pivotDataQuery, 0, 'pivot_rows'));
		$pivot_datafield = explode(',', $adb->query_result($pivotDataQuery, 0, 'pivot_datafield'));

		$pivotColumns = array();
		foreach ($pivot_columns as $key => $value) {
			$pivotColumns[$value] = $value;
		}
		$viewer->assign('pivotColumns', $pivotColumns);

		$pivotRows = array();
		foreach ($pivot_rows as $key => $value) {
			$pivotRows[$value] = $value;
		}
		$viewer->assign('pivotRows', $pivotRows);

		$pivotDatafield = array();
		foreach ($pivot_datafield as $key => $value) {
			$pivotDatafield[$value] = $value;
		}
		$viewer->assign('pivotDatafield', $pivotDatafield);
		//Pivot Report
	...
	function step3(Vtiger_Request $request) {
		...
		//Pivot Report
		$type = $request->get('type');
		if(is_array($request->get('pivot_rows'))){
			$pivot_rows = implode(',', $request->get('pivot_rows'));
		}else{
			$pivot_rows = $request->get('pivot_rows');
		}
		if(is_array($request->get('pivot_columns'))){
			$pivot_columns = implode(',', $request->get('pivot_columns'));
		}else{
			$pivot_columns = $request->get('pivot_columns');
		}
		if(is_array($request->get('pivot_datafield'))){
			$pivot_datafield = implode(',', $request->get('pivot_datafield'));
		}else{
			$pivot_datafield = $request->get('pivot_datafield');
		}
		//Pivot Report
		...
		*/ 
		...
		//Pivot Report
		$viewer->assign('type', $type);
		$viewer->assign('pivot_rows', $pivot_rows);
		$viewer->assign('pivot_columns', $pivot_columns);
		$viewer->assign('pivot_datafield', $pivot_datafield);
		//Pivot Report
