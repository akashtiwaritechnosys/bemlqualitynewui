...
class Reports_Detail_View extends Vtiger_Index_View {
	...
	function preProcess(Vtiger_Request $request, $display=true) {
		...
		if($report_type == 'Pivot'){
			$editViewURL = 'index.php?module=Reports&view=Edit&record='.$recordId.'&type='.$report_type;
			$viewer->assign('editViewURL', $editViewURL);
		}

		$reportModuleQuery = $adb->pquery("SELECT * FROM vtiger_reportmodules WHERE reportmodulesid = ?", array($recordId));

		$pivotDataQuery = $adb->pquery("SELECT * FROM vtiger_pivot_report_fields WHERE reportid = ?", array($recordId));
		...
		$groupByFields = ' GROUP BY ';
		$groupBySelectFields = '';
		foreach ($pivot_rows as $key => $value) {
			$fieldValue = explode(':', $value);
			$groupByFields .= ' '.$fieldValue[0].'.'.$fieldValue[1].',';
			$groupBySelectFields .= ','.$fieldValue[0].'.'.$fieldValue[1].',';
		}

		$selectFields = ' ';
		foreach ($pivot_datafield as $key => $value) {
			$fieldValue = explode(':', $value);
			if (strpos($fieldValue[4], 'SUM') !== false) {
			    $selectFields .=' SUM('.$fieldValue[0].'.'.$fieldValue[1].'),';
			}
		}

		$explodeLimitQuery = explode('from', $sSQL);
		$pivotQuery = 'SELECT '.rtrim($selectFields, ',').rtrim($groupBySelectFields, ',').' FROM '.$explodeLimitQuery[1];
	
		$explodeLimitQuery = explode('LIMIT', $pivotQuery);
		$finalpPivotQuery = $explodeLimitQuery[0].' '.rtrim($groupByFields, ',');

		global $adb;
		echo $finalpPivotQuery;exit;
		$pivotResult = $adb->pquery($finalpPivotQuery);
		$pivotRows = $adb->num_rows($pivotResult);*/
		//Pivot Report
