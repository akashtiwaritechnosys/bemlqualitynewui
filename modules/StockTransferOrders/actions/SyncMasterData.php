...
class StockTransferOrders_SyncMasterData_Action extends Vtiger_IndexAjax_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if ($currentUserModel->isAdminUser()) {
			...
			// 	++$sortid;
			// 	$fieldId =  $fieldName . 'id';
			// 	$sql = "INSERT INTO vtiger_$fieldName( $fieldId , $fieldName, presence,sortorderid,code,plant_code) VALUES(?,?,?,?,?,?)";
			// 	$arr = array($new_id, trim($value->{'LGORT'}) . ' - ' . $val, 1, $sortid, trim($value->{'LGORT'}), trim($value->{'WERKS'}));
			// 	$adb->pquery($sql, $arr);
			// }
