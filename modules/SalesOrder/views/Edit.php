	...
	public function process(Vtiger_Request $request) {
		...
		if ($request->has('totalProductCount')) {
			...
			if (!empty($external_app_num)) {
				...
				$viewer->assign('MESSAGE', 'SalesOrder Is Already Synced To SAP');
			...
			$currencyInfo = $parentRecordModel->getCurrencyInfo();
			$taxes = $parentRecordModel->getProductTaxes();
			$relatedProducts = $parentRecordModel->getProductsForSo($request);
			$recordModel = Vtiger_Record_Model::getCleanInstance($moduleName);

			$equipmentId = $parentRecordModel->get('equipment_id');
			if (!empty($equipmentId)) {
				$EquipmentRecordModel = Vtiger_Record_Model::getInstanceById($equipmentId);
				$soldToParty = $EquipmentRecordModel->get('sold_to_party');
				$shipToParty = $EquipmentRecordModel->get('ship_to_party');
				if(empty($soldToParty) && !empty($shipToParty)){
					$recordModel->set('sold_to_party', $shipToParty);
					$recordModel->set('sold_to_party', $shipToParty);
				} else if(!empty($soldToParty) && empty($shipToParty)){
					$recordModel->set('sold_to_party', $soldToParty);
					$recordModel->set('ship_to_party', $soldToParty);
				} else {
					$recordModel->set('sold_to_party', $soldToParty);
					$recordModel->set('ship_to_party', $shipToParty);
				}
			}
			$recordModel->setRecordFieldValues($parentRecordModel);
		} else {
