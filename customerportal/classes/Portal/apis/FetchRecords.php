...
class Portal_FetchRecords_API extends Portal_Default_API {
	...
	public function process(Portal_Request $request) {
		...
		$result = Vtiger_Connector::getInstance()->fetchRecords($module, $request->get('label'), $request->get('q', array()), $params['fields'], $pageNo, $pageLimit,false,false ,$request->get('statusFilterName'));
	...
	public function processResponse($result, $module, $language, $isExport = false) {
		...
		foreach ($headers as $key) {
			if (empty($key) || empty($recordMeta[$key]['label'])) {
				continue;
			}
			$headerData = [];
