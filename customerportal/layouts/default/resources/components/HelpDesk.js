...
function HelpDesk_EditView_Component($scope, $modalInstance, record, api, webapp, module, $timeout, $translatePartialLoader, language, $filter, $http, editStatus) {
	...
	$scope.setOtherREferenceFieldValues = function (recordId, module) {
		...
		$http.get('index.php?module=' + module + '&api=FetchRecord', {
			...
			$scope.data['sr_equip_model'] = response.data.result.recordInfo.equip_model;
			$('.sr_equip_model option[value="'+ $scope.data['sr_equip_model'] +'"]').attr("selected", "selected").trigger('change').prop("disabled", true);
			$scope.handleHMROrKMDA(response.data.result.recordInfo.eq_last_hmr, response.data.result.recordInfo.eq_last_km_run);
		});
	}
	...
	$scope.handleHMROrKMDA = function (lastHMR, lastKM) {
		...
		if (isNaN(lastHMR) || lastHMR == '') {
			...
			if ($('#idofSpanhmrEle').length) {
				...
				$("#hmrhideOrShowInputId").prepend("<span id='idofSpanhmrEle'><b> Last HMR Value : " + lastHMR + "</b></span>");
			...
			$scope.IsHMRORKM = 'HMR';
			$scope.lastHMR = lastHMR;
			$scope.lastKM = 0;

			jQuery('input[name="hmr"]').attr('min', lastHMR);
			$('#minErrorHolderhmr').html('HMR Needs To Be gretaer than previous HMR : ' + lastHMR);
			jQuery('#kilometer_readingIdEle').val('');
			jQuery('#kilometer_readingIdEle').prop("disabled", true);
			jQuery('#hmrIdEle').prop("disabled", false);
			jQuery('#hmrIdEle').attr("required", 'required');
			jQuery('#hmrIdEle').attr("ng-required", true);
		}
		...
		if (isNaN(lastKM) || lastKM == '') {
			...
			jQuery('#hmrIdEle').prop("disabled", true);
			jQuery('#kilometer_readingIdEle').prop("disabled", false);
			jQuery('#kilometer_readingIdEle').attr("required", 'required');
			jQuery('#kilometer_readingIdEle').attr("ng-required", true);
			$('#minErrorHolderkilometer_reading').html('KM reading Needs To Be gretaer than previous KM reading : ' + lastKM);
		}
	}
	...
	$scope.handledependency = function ($e, $value, $api, $webapp) {
		...
		if (fieldName == 'ticket_type' || fieldName == 'purpose') {
			...
			if (fieldName == 'ticket_type') {
				...
				jQuery('#func_loc_idIdEle').prop("readonly", true);
				$("#elesr_equip_model").select2({
					// Select2 options
	...
	$scope.fetchReferenceRecords = function (module, query) {
		var records = [];
		let selectedType = $("select#ticket_type option").filter(":selected").text();
		return $http.get('index.php?module=' + module + '&api=FetchReferenceRecords' + '&selectedType=' + selectedType, {
			params: {
				module: module,
				query: query
			}
	...
	$scope.save = function (validity, dateFilter) {
		...
		if (params['record']['equip_id_da'] == 'Other') {
			params['record']['equip_id_da'] = null;
		}
